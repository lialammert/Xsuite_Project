...
xshifts = [0]
yshift = 0
couplings = [0,-4,-4.5,-3] #[0,-2] # try 1e-2
    ...
    for coupling in couplings :
        ...
        for bbstate in beambeamstates:
            ...
            arc_b1 = xt.LineSegmentMap(betx = beta_x,bety = beta_y,
                    qx = Qx-2*qxchange, qy = Qy-2*qychange,bets = beta_s, qs=Qs) # note that this is only half // -2*qxchange, -2*qychange
            arc = xt.LineSegmentMap(betx = beta_x,bety = beta_y,
                    qx = qxchange, qy = qychange, bets = beta_s, qs=Qs)
            arc2 = xt.LineSegmentMap(betx = beta_x,bety = beta_y,
                    qx = qxchange, qy = qychange, bets = beta_s, qs=Qs)
            #quad = xt.Quadrupole(k1=0, k1s = ksl, length  = 1)
            ...
            #quad2 = xt.Quadrupole(k1=0, k1s = -ksl, length  = 1)
            ...
            #monitor_ip = xt.ParticlesMonitor(start_at_turn=0, stop_at_turn=nTurn,num_particles=n_macroparticles)
            ...
            monitor_3 = xt.ParticlesMonitor(start_at_turn=0, stop_at_turn=nTurn,num_particles=n_macroparticles)
            monitor_4 = xt.ParticlesMonitor(start_at_turn=0, stop_at_turn=nTurn,num_particles=n_macroparticles)
            #monitor_ip_2 = xt.ParticlesMonitor(start_at_turn=0, stop_at_turn=nTurn,num_particles=n_macroparticles)
            elements_b1 = [quad,arc,bbeam_b1, arc2, quad2, arc_b1]#[quad,arc,bbeam_b1, arc, quad2, arc_b1] #[bbeam_b1, arc, quad, arc_b1, quad2, arc]
            elements_b2 = [quad, arc_b2, quad2, arc, bbeam_b2, arc] # [bbeam_b2,arc_b2]#[bbeam_b2, arc, quad2, arc_b2, quad, arc]#[quad, arc_b2, quad2, arc, bbeam_b2, arc] #[bbeam_b2, arc_b2] #[bbeam_b2, quad, arc_b2]
            line_b1 = xt.Line(elements=elements_b1)
            line_b1.insert_element(element=monitor_1, name='m1',index=0)
            line_b1.insert_element(element=monitor_2, name='m2',index=3)
            line_b1.insert_element(element=monitor_3, name='m3',index=5)
            line_b1.insert_element(element=monitor_4, name='m4',index=8)
            print(line_b1.element_names)
            line_b2 = xt.Line(elements=elements_b2)
            #line_b2.insert_element(element=monitor_ip_2, name='IP1_2',index=1)
            ...
            # line_b1.track(particles_b1, num_turns=nTurn,turn_by_turn_monitor=True)
            # line_b2.track(particles_b2,num_turns=nTurn,turn_by_turn_monitor=True)
            # x1.append(monitor_ip.x)
            # y1.append(monitor_ip.y)
            # x2.append(monitor_ip_2.x)
            # y2.append(monitor_ip_2.y)
            # px1.append(monitor_ip.px)
            # py1.append(monitor_ip.py)
            # px2.append(monitor_ip_2.px)
            # py2.append(monitor_ip_2.py)
            ...
            x1_3.append(monitor_3.x)
            y1_3.append(monitor_3.y)
            px1_3.append(monitor_3.px)
            py1_3.append(monitor_3.py)
            x1_4.append(monitor_4.x)
            y1_4.append(monitor_4.y)
            px1_4.append(monitor_4.px)
            py1_4.append(monitor_4.py)
 
            # line_b1.track(particles_b1, num_turns=nTurn,
